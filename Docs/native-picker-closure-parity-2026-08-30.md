...
| Header, price/category legend, category filters, test mode, attribution | Public native components use snapshot truth. Test Mode remains visible independently. Attribution is outside the builder matrix and appears only when runtime `branding.attributionRequired` is true; host builders cannot override either branch. | Covered |
...
| Whole-component replacements/builders | Exactly 25 part IDs; missing or throwing builder renders safe `defaultContent`. Test truth and API-required attribution are not replaceable. | Gap fixed |
